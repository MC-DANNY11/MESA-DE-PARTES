...
if(!empty($_POST['codigo']) && !empty($_POST['nombre'])){
    ...
    $codigo = $_POST['codigo'];
    $nombre = $_POST['nombre'];
    $telefono = $_POST['telefono'] ?? '';  // Asegurar que la descripción no sea nula
    $direccion = $_POST['direccion'] ?? '';  // Asegurar que la descripción no sea nula
    $fecha = date("Y-m-d H:i:s");

    // Actualizar la caja
    $u_caja = "UPDATE tbleditoriales SET nombre=?, telefono=?, direccion=? WHERE idEditorial=?";
    $stmt_caja = $con->prepare($u_caja);
    $resultado = $stmt_caja->execute([$nombre, $telefono,$direccion,$codigo]);

    if($resultado){
        // Registrar en el historial
        $id_registro = $codigo;  // El ID de la caja actualizada
        $tblafectada = "Editorial";
        $t_cambio = "Se actualizó datos de la Editorial";
